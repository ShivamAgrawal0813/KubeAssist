...
def parse_pdf(file_path: str) -> str:
    ...
    Fallbacks to pdfplumber for pages that yield no text (e.g. image heavy pages).
    """
    with logfire.span("PDF parsing (local)", filename=file_path):
        try:
            ...
            if blank_pages:
                logfire.info(f"pydf retruned blank on pages {blank_pages} - retrying with pdfplumber.")
                try:
                    import pdfplumber
